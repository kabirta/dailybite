...
function CaloriesPanel({ report }: { report: any }) {
...
function MacrosPanel({ report }: { report: any }) {
  const totals = report?.totals ?? {};
  const macroRows = [
    { label: "Carbohydrate", color: "#4DA8D8", total: `${Math.round(totals.carbs ?? 0)}g`, goal: "50%" },
    { label: "Fat", color: "#D3A017", total: `${Math.round(totals.fat ?? 0)}g`, goal: "30%" },
    { label: "Protein", color: "#C9757E", total: `${Math.round(totals.protein ?? 0)}g`, goal: "20%" },
  ];
  return (
    <View style={{ gap: 12 }}>
      {/* Macros chart card */}
...
function NutrientsPanel({ report }: { report: any }) {
  const totals = report?.totals ?? {};
  const nutrientRows = [
    { label: "Calories (kcal)", total: Math.round(totals.calories ?? 0), goal: "3000", delta: "-" },
    { label: "Protein (g)", total: Math.round(totals.protein ?? 0), goal: "150", delta: "-" },
    { label: "Carbohydrate (g)", total: Math.round(totals.carbs ?? 0), goal: "375", delta: "-" },
    { label: "Fiber (g)", total: Math.round(totals.fiber ?? 0), goal: "30", delta: "-" },
    { label: "Sugar (g)", total: Math.round(totals.sugar ?? 0), goal: "-", delta: "-" },
    { label: "Fat (g)", total: Math.round(totals.fat ?? 0), goal: "100", delta: "-" },
    ...
    { label: "Water (ml)", total: Math.round(totals.water ?? 0), goal: "-", delta: "-" },
  ];
  return (
    <Card>
...
export default function ReportsScreen() {
  ...
  useEffect(() => {
    let isActive = true;
    getDailyNutritionReport(new Date())
      .then((data) => {
